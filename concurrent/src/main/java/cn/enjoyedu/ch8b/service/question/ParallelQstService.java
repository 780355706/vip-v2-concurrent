...
public class ParallelQstService {
    ...
    /**
     * 为了定期的清除缓存中的信息，使用谷歌为我们提供的ConcurrentLinkedHashMap，它使用了LRU（最近最少使用原则）策略的缓存。
     * 如果缓存中的题目越少使用，这个题目离我们当前考试考察的范围就越远，被再次选中的概率就越小。所以ConcurrentLinkedHashMap
     * 可以设定一个初始大小，可以设定为2000~5000或其他，当map满了之后，就把当前容器中使用频率最低的数据替换掉。
     */
    /*题目在本地的缓存*/
